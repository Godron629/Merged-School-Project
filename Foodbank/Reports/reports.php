		...
		switch($_POST['reportType'])
		{
			...
			{	
				if(isset($_POST['paramInclude']))
				{
					...
					if($_POST['paramInclude'] == "showAll")
					{
						...
						{
							$test = strtotime("00:00:00") - strtotime("00:00:00");
						}
						
						$sql2 = "select volunteer_id, volunteer_fname, volunteer_lname from volunteer where volunteer_id='" . $_POST['people'] . "'";
						$result2 = $conn->query($sql2);
						$volunteer = $result2->fetch_assoc();
						
						
						$reportAppend .="<b>" . $volunteer['volunteer_id'] . " : " .  $volunteer['volunteer_fname'] . " " . $volunteer['volunteer_lname'] . "</b> has worked for a total of: <b>" . gmdate("H:i:s", $test) . "</b> during their time with us.";
					...
					{
						if($_POST['firstDate'] != null && $_POST['secondDate'] != null)
						{
							if($_POST['firstDate'] < $_POST['secondDate'])
							{
								...
								$reportAppend .="<b>" . $volunteer['volunteer_id'] . " : " .  $volunteer['volunteer_fname'] . " " . $volunteer['volunteer_lname'] . "</b> has worked for a total of: <b>" . gmdate("H:i:s", $test) . "</b> during the dates of " .$_POST['firstDate'] . " and ".$_POST['secondDate'] . ".";
							}	
							else
							{
								$reportAppend .= "Dates entered out of order. Please ensure the first date is BEFORE the second.";
							}
			...
			case "report5":
			{
				if(isset($_POST['paramInclude']))
				{
					...
					if($_POST['paramInclude'] == "showAll")
					{
						$sql = "select clock_hours_worked from clock_entry";
						$result = $conn->query($sql);
						$calc = strtotime("00:00:00");
						$test = "";				
						...
						$reportAppend .="All volunteers have worked for a total of: <b>" . gmdate("H:i:s", $test) . "</b> during their time with us.";
					...
					else
					{
						if($_POST['firstDate'] != null && $_POST['secondDate'] != null)
						{
							if($_POST['firstDate'] < $_POST['secondDate'])
							{
								$sql = "select clock_hours_worked from clock_entry where clock_day_worked BETWEEN '". $_POST['firstDate'] ."' AND '" . $_POST['secondDate'] . "'";
								$result = $conn->query($sql);
								$calc = strtotime("00:00:00");
								$test = "";				
								...
								$reportAppend .= "All volunteers have worked for a total of: <b>" . gmdate("H:i:s", $test) . "</b> during the dates of " .$_POST['firstDate'] . " and ".$_POST['secondDate'] . ".";
							}	
							else
							{
								$reportAppend .= "Dates entered out of order. Please ensure the first date is BEFORE the second.";
			...
			{	
				...
				if(isset($_POST['reportType']))
				{
					...
					for($i = 0; $i < sizeof($arrayCounter); $i++)
					{
						for($j = 0; $j < sizeof($acceptedParams2); $j++)
						{
							$sqlDept = "select volunteer_fk from pref_dept where {$acceptedParams2[$j]} AND volunteer_fk='{$arrayCounter[$i]}'";
					...
					for($i = 0; $i < sizeof($arrayCounter2); $i++)
					{
						...
						{
							$sql = "select volunteer_fname, volunteer_lname from volunteer where volunteer_id='" .$holderArray[$i] ."'";
							$result = $conn->query($sql);
							while($row = $result->fetch_assoc())
							{
			...
			case "report8":
			{
				...
				{
					$reportAppend = "";
					$sql = "select volunteer_email from volunteer;";
					$result = $conn->query($sql);
				
					while($row = $result->fetch_assoc())
					{
						$report = $row['volunteer_email'] . "<br>";
						$reportAppend .= $report;
					}
				...
				{
					$reportAppend = "";
					$sql = "select volunteer_email from volunteer where volunteer_status = 1;";
					$result = $conn->query($sql);
				...
				else if(isset($_POST['paramInclude']) && $_POST['paramInclude'] == 'show' && isset($_POST['people']))
				{
					...
					foreach ($_POST['people'] as $selectedOption)
					{
						$sql = "select volunteer_email from volunteer where volunteer_id ='" . $selectedOption . "';";
						$result = $conn->query($sql);
...
<script>
function displayReport45(isSelected, inputVal)
{
	if(isSelected == true && inputVal == "showAll")
	{
...
<script>
function displayReport5(isSelected, inputVal)
{
	if(isSelected == true && inputVal == "showAll")
	{
		document.getElementById('specific').hidden = true;
